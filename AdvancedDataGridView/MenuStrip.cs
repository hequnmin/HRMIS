                ...
                if (DataType == typeof(DateTime))
                {
                    foreach (TreeNodeItemSelector n in nodes)
                    {
                        if (n.Checked && (n.Nodes.AsParallel().Cast<TreeNodeItemSelector>().Where(sn => sn.CheckState != CheckState.Unchecked).Count() == 0))
                        {
                            ...
                            sb.Append("(Convert([{0}], 'System.String') LIKE '%" + (IsFilterDateAndTimeEnabled ? dt : dt.Date).ToString("yyyy-MM-dd HH") + "%')" + appx);
                        }
        ...
        private void BuildNodes(IEnumerable<DataGridViewCell> vals)
        {
            ...
            if (vals != null)
            {
                ...
                if (vals.Count() > 0)
                {
                                ...
                                foreach (var day in days)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        foreach (var hour in hours)
                                        {
                                            ...
                                            //    }
                                            //}
                                            TreeNodeItemSelector hoursnode = daysnode.CreateChildNode(hour.Key.ToString("D2") + " " + "h", hour.First().Value);
                                        }
                                    }
                                }
                            }
                    ...
                    //add ip nodes 2022-09-29 Min
                    else if (DataType == typeof(System.Net.IPAddress))
                    {
                        foreach (var v in nonulls.GroupBy(c => c.Value))
                        {
                            TreeNodeItemSelector node = TreeNodeItemSelector.CreateNode(v.First().FormattedValue.ToString(), v.Key.ToString(), CheckState.Checked, TreeNodeItemSelector.CustomNodeType.Default);
                            ChecklistAddNode(node);
                        }
                    }
